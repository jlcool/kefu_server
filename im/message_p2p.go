...
func MessageP2P(message models.Message) {
	...
	if message.BizType == "search_knowledge" {
		...
		if isTransfer {
			...
			_, _ = o.Raw("SELECT a.*, IFNULL(c.count,0) AS `count` FROM admin as a LEFT  JOIN (SELECT to_account,COUNT(*) AS count FROM `contact` WHERE is_session_end = 0 GROUP BY to_account) c ON a.id = c.to_account WHERE a.`online` = 1 and a.`id`!=1 ORDER BY c.count").QueryRows(&admins)
			if len(admins) <= 0 {
